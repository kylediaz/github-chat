...
export const githubRepo = pgTable(
  ...
  (table) => ({
    ownerRepoIdx: uniqueIndex("github_repo_owner_repo_idx").on(
      table.owner,
      table.repo,
    ),
  }),
...
export const githubRepoCommit = pgTable(
  ...
  (table) => ({
    ...
    ownerRepoIdx: index("github_repo_commit_owner_repo_idx").on(
      table.owner,
      table.repo,
    ),
...
export const githubRepoTrees = pgTable(
  ...
  (table) => ({
    ownerRepoTreeShaIdx: uniqueIndex(
      "github_repo_trees_owner_repo_tree_sha_idx",
    ).on(table.owner, table.repo, table.treeSha),
    ownerRepoIdx: index("github_repo_trees_owner_repo_idx").on(
      table.owner,
      table.repo,
    ),
  }),
...
export const githubSyncSources = pgTable(
  ...
  (table) => ({
    ownerRepoIdx: uniqueIndex("github_sync_sources_owner_repo_idx").on(
      table.owner,
      table.repo,
    ),
  }),
...
export const githubSyncInvocations = pgTable(
  ...
  (table) => ({
    sourceUuidIdx: index("github_sync_invocations_source_uuid_idx").on(
      table.sourceUuid,
    ),
    statusIdx: index("github_sync_invocations_status_idx").on(table.status),
    createdAtIdx: index("github_sync_invocations_created_at_idx").on(
      table.createdAt,
    ),
  }),
);
