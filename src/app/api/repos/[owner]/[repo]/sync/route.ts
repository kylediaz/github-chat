...
async function computeStatusFromData(
  ...
  if (!commit) {
    commit =
      (await db.query.githubRepoCommit.findFirst({
        where: and(
...
export async function POST(
  ...
  try {
    ...
    let shouldFetchRepo =
      !repoData || (repoData.fetchedAt && repoData.fetchedAt < oneDayAgo);
    ...
    if (shouldFetchRepo) {
      ...
      if (
        "accessible" in ghRepo &&
        ghRepo.accessible === false &&
        ghRepo.private === true
      ) {
        await db
          .insert(githubRepo)
    ...
    let shouldFetchCommit =
      !latestCommitInDb ||
      (latestCommitInDb.fetchedAt && latestCommitInDb.fetchedAt < oneDayAgo);
    ...
    if (shouldFetchCommit) {
      ...
      const ghCommit = await getBranchCommit(
        owner,
        repo,
      ...
      if (!ghCommit) {
        const errorResponse: ErrorResponse = {
          error: "Failed to fetch branch commit",
        };
        return NextResponse.json(errorResponse, { status: 500 });
      }
      ...
      const [insertedCommit] = await db
        ...
        .onConflictDoUpdate({
          target: [
            githubRepoCommit.owner,
            githubRepoCommit.repo,
      ...
      if (!existingTree) {
        ...
        if (ghTree) {
          await db
            ...
            .onConflictDoUpdate({
              target: [
                githubRepoTrees.owner,
                githubRepoTrees.repo,
                githubRepoTrees.treeSha,
              ],
              set: {
                tree: ghTree.tree,
              },
    ...
    let source: typeof githubSyncSources.$inferSelect | null =
      sourceResult ?? null;

    const [latestInvocation, completedInvocation, latestCommit] =
      await Promise.all([
        source
      ...
      try {
        ...
        if (errorCode === "23505") {
          ...
          if (existingSource) {
            ...
            const errorResponse: ErrorResponse = {
              error: "Failed to create Chroma source",
            };
            return NextResponse.json(errorResponse, { status: 500 });
          }
        } else {
    ...
    try {
      ...
      await db.insert(githubSyncInvocations).values({
      ...
      const errorResponse: ErrorResponse = {
        error: "Failed to create Chroma invocation",
      };
      return NextResponse.json(errorResponse, { status: 500 });
    }
    ...
    const updatedLatestInvocation =
      await db.query.githubSyncInvocations.findFirst({
        where: eq(githubSyncInvocations.sourceUuid, source.uuid),
        orderBy: [desc(githubSyncInvocations.createdAt)],
      });

    const updatedCompletedInvocation =
      await db.query.githubSyncInvocations.findFirst({
        where: and(
          eq(githubSyncInvocations.sourceUuid, source.uuid),
          eq(githubSyncInvocations.status, "completed"),
